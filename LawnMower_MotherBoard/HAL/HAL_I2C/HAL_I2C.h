/*
 * HAL_I2C.h
 *
 *  Created on: 12 sept. 2022
 *      Author: morgan.venandy
 */

#ifndef HAL_HAL_I2C_HAL_I2C_H_
#define HAL_HAL_I2C_HAL_I2C_H_

#include <stdint.h>

typedef enum 
{
	E_SLAVE_READ_DATA_V,
	E_SLAVE_READ_DATA_A,
	E_SLAVE_READ_DATA_DOCK,
	E_SLAVE_READ_DATA_SONAR_FC,
	E_SLAVE_READ_DATA_SONAR_FL,
	E_SLAVE_READ_DATA_SONAR_FR,
	
	E_SLAVE_READ_DATA_NUMBER
}E_SLAVE_READ_DATA;

typedef enum 
{
    E_ESP32_READ_DATA_GPS_LONG_MMSB,
    E_ESP32_READ_DATA_GPS_LONG_MSB,
    E_ESP32_READ_DATA_GPS_LONG_LSB,
    E_ESP32_READ_DATA_GPS_LONG_LLSB,
    E_ESP32_READ_DATA_GPS_LAT_MMSB,
    E_ESP32_READ_DATA_GPS_LAT_MSB,
    E_ESP32_READ_DATA_GPS_LAT_LSB,
    E_ESP32_READ_DATA_GPS_LAT_LLSB,
    E_ESP32_READ_DATA_RTK_FIX,
    E_ESP32_READ_DATA_TIME_TO_MOW,
    E_ESP32_READ_DATA_ENROLLED,
	
	E_ESP32_READ_DATA_NUMBER
}E_ESP32_READ_DATA;

typedef enum 
{
	E_SLAVE_WRITE_DATA_LED_STATUS = E_SLAVE_READ_DATA_NUMBER,
	
	E_SLAVE_WRITE_DATA_NUMBER
}E_SLAVE_WRITE_DATA;

typedef enum 
{
	E_ESP32_WRITE_DATA_LED_STATUS = E_ESP32_READ_DATA_NUMBER,
    E_ESP32_WRITE_DATA_AZIMUT_MSB,
    E_ESP32_WRITE_DATA_AZIMUT_LSB,
	
	E_ESP32_WRITE_DATA_NUMBER
}E_ESP32_WRITE_DATA;

void HAL_I2C_Init(void);
void HAL_I2C_Write(void);
uint8_t HAL_I2C_Read(uint8_t* , uint8_t* );
void HAL_I2C_ReadAccel(int16_t* pu16_pitch, int8_t* pu8_roll);
uint16_t HAL_I2C_ReadCompass(void);
uint8_t HAL_I2C_ReadSlave(uint8_t* pu8_RxBuff, uint8_t* pu8_Size);
uint8_t HAL_I2C_WriteSlave(uint8_t u8_mowerState);
uint8_t HAL_I2C_ReadESP32(uint8_t* pu8_RxBuff, uint8_t* pu8_Size);
uint8_t HAL_I2C_WriteESP32(uint8_t u8_mowerState, uint16_t u16_azimut);

#endif /* HAL_HAL_I2C_HAL_I2C_H_ */
